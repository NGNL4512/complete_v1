...
for ii in range(0,3):
    ...
    for i in range(ts):
        ...
        totalvolume11=totalvolume11/ts
        totalvar11=totalvar11/ts
        totaltalkspeed11=totaltalkspeed11/ts
        totalnospeak11=totalnospeak11/ts
    if(ii==0):
        normal1.append(endd)
        normal1.append(var)
...
normal=(normal1[0]+normal2[0]+normal3[0])/3
normall=(normal1[1]+normal2[1]+normal3[1])/3
normalll=(normal1[2]+normal2[2]+normal3[2])/3

nnormal=(normal11[0]+normal22[0]+normal33[0])/3
nnnormal=(normal11[1]+normal22[1]+normal33[1])/3
nnnnormal=(normal11[2]+normal22[2]+normal33[2])/3
nnnnnormal=(normal11[3]+normal22[3]+normal33[3])/3
...
def text_cw(name, msg):
    ...
    #file.write(msg)
    file.close()
value=np.array([["完整音量平均:"+str(normal).replace('[','').replace(']','')]
                ,["完整音量變異數:"+str(normall).replace('[','').replace(']','')]
                ,["完整語速:"+str(normalll).replace('[','').replace(']','')]
                ,["斷句音量平均:"+str(nnormal).replace('[','').replace(']','')]
                ,["斷句音量變異數:"+str(nnnormal).replace('[','').replace(']','')]
                ,["斷句語速:"+str(nnnnormal).replace('[','').replace(']','')]
                ,["斷句沉默時間:"+str(nnnnnormal).replace('[','').replace(']','')]])
text_cw("a",value)
